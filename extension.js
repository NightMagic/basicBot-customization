(function () {
    ...
    function extend() {
        ...
        bot.commands.streamCommand = {
            ...
            functionality: function (chat, cmd) {
                ...
                if (!bot.commands.executable(this.rank, chat)) return void (0);
                    ...
                    API.sendChat("/me Go check out Skyasi's stream! http://www.twitch.tv/skyasi");
                }
            }
        };
        ...
        bot.commands.subCommand = {
            command: 'sub',  //The command to be called. With the standard command literal this would be: !bacon
            rank: 'user', //Minimum user permission to use the command
            type: 'exact', //Specify if it can accept variables or not (if so, these have to be handled yourself through the chat.message
            functionality: function (chat, cmd) {
                if (this.type === 'exact' && chat.message.length !== cmd.length) return void (0);
    ...
    $.getScript('https://rawgit.com/benzi/basicBot/master/basicBot.js', extend);
